...
/**
 * ClassName: Multiple_AddImageWatermarkToPDF
 * Package: org.example
 * Description:
 *
 * @Author: 闫守瑞
 * @Create: 2023/9/23 - 9:21
 * @Version: v1.0
 */
 ...
 * @Version: v1.0
 */
public class Multiple_AddImageWatermarkToPDF {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 循环为每页添加水印
            int numWatermarksPerPage = 4; // 每页水印数量
            int numWatermarksPerRow = 2; // 每行水印数量

            for (int i = 1; i <= numPages; i++) {
                // 获取页面
                PdfCanvas canvas = new PdfCanvas(pdfDoc.getPage(i));

                for (int j = 0; j < numWatermarksPerPage; j++) {
                    // 创建ImageData对象
                    ...
                    // 根据j的值调整水印位置
                    x += (j % numWatermarksPerRow) * watermarkImage.getImageScaledWidth();
                    y += (j / numWatermarksPerRow) * watermarkImage.getImageScaledHeight();
